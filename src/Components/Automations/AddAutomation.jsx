...
import { FaSms, FaTemperatureHigh, FaEnvelope, FaCaretUp, FaCaretDown, FaArrowRight } from "react-icons/fa";
import { IoIosLock } from "react-icons/io";
import { LuAlarmClock } from "react-icons/lu";

const automationActions = [
    { icon: <MdTextsms className='bg-lblue p-2 rounded-3 text-blue fs-1' />, label: 'Send In-App Message' },
    { icon: <FaSms className='bg-lblue p-2 rounded-3 text-blue fs-1' />, label: 'Send SMS' },
    { icon: <FaTemperatureHigh className='bg-lblue p-2 rounded-3 text-blue fs-1' />, label: 'Set Temperature' },
    { icon: <IoIosLock className='bg-lblue p-2 rounded-3 text-blue fs-1' />, label: 'Set Lock' },
    { icon: <LuAlarmClock className='bg-lblue p-2 rounded-3 text-blue fs-1' />, label: 'Change House Status' },
    { icon: <FaEnvelope className='bg-lblue p-2 rounded-3 text-blue fs-1' />, label: 'Gmail' },
];
    ...
    return (
        <div className="w-100 position-relative">
            ...
            >
                ...
                {isOpen ? <FaCaretUp className='text-blue' /> : <FaCaretDown className='text-blue' />}
            </div>
            ...
            {isOpen && (
                <div className='shadow bg-white p-2 rounded-3 mt-3 position-absolute w-100 top-100 start-0 index-10'>
                    <div className='shadow rounded-3 d-flex align-items-center p-2 border border-info'>
                        <label htmlFor="searchInput"><CiSearch className='bg-blue p-2 rounded-3 text-white fs-1' /></label>
                        <input type="search" className='w-100 border-0 px-3' id='searchInput' placeholder='Search' />
                    </div>
                    ...
                    {automationActions.map((action, index) => (
                        <div
                            ...
                            onClick={() => onSelectAction(action.label)} // Trigger action selection
                        >
                            {action.icon}
...
function AddAutomation() {
    ...
    const handleClickOutside = (event) => {
        if (notificationRefWhen.current && !notificationRefWhen.current.contains(event.target)) {
            setIsNotificationOpenWhen(false);
        }
        if (notificationRefThen.current && !notificationRefThen.current.contains(event.target)) {
                                    ...
                                    {isNotificationOpenWhen && (
                                        <div ref={notificationRefWhen} className='bg-white border shadow rounded-3 p-3 mt-3 max position-relative top-0 start-0 z-index-10'>
                                            ...
                                            <div className='d-flex my-2'>
                                                ...
                                                <label htmlFor="" className='d-flex align-items-center ms-2'>
                                                    <div>
                                                        <p className='mb-0 text-nowrap'>When date arrives at</p>
                                                    </div>
                                                    <div className='ms-2'>
                                                        <select name="" id="" className='form-select shadow-none bg-lgrey'>
                                            ...
                                            <div className='d-flex mb-2'>
                                                ...
                                                <label htmlFor="" className='d-flex align-items-center ms-2'>
                                                    <div>
                                                        <input type="text" className='form-control bg-lgrey shadow-none in-w' placeholder='5' />
                                                    </div>
                                                    <div className='ms-2'>
                                                        <select name="" id="" className='form-select shadow-none bg-lgrey in-s'>
                                                    ...
                                                    <div className='ms-2'>
                                                        <p className='text-nowrap mb-0'> dates arrive, at</p>
                                                    </div>
                                                    <div className='ms-2'>
                                                        <select name="" id="" className='form-select shadow-none bg-lgrey in-s'>
                                                            <option value="">09:30 PM</option>
